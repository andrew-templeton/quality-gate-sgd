...
The [prepared external reader study](https://github.com/andrew-templeton/isogloss/tree/02e6630e0cfda250e294ad026ad11bb0137791ff/studies/rendered-reader) freezes the current baseline and disclosure layouts for twelve eligible business readers, with six first exposures per layout. Its staged player records the actual iframe viewport and keeps the other layout/source outside the first-exposure phase. Eight geometry comparisons matched; seven screenshot pairs were byte-identical and one disclosure-control discrepancy is retained. This exploratory single-case study has model-authored reference labels and zero real responses; it does not estimate individual-facet effects or general reader utility.
