...
Reviewed September 14, 2026, for development version `2.0.0-dev.0`.
...
| External assertion runners | Hardened; regression tests pass | Compilation preserves a valid runner's receiver and prevents replacement of the compiled runner. General configuration identity still needs work. |
...
| P2 | Changing configuration captured by an evaluator changed the same input from fail to pass with identical input and contract digests. See [contracts](../../src/v2/types.ts), [compilation](../../src/v2/catalog.ts) and [evaluation identity](../../src/v2/evaluate.ts). | The render-specific policy gap is addressed; the general implementation/configuration identity contract remains open before stable interoperability. |
...
| P2 | Schema/capability strings were used for discovery, while every evaluator received `unknown` and implemented its own parsing. See [input contracts](../../src/v2/types.ts) and [discovery](../../src/v2/catalog.ts). | Open before stable interoperability: provide an executable, typed input binding contract. |
| P2 | Ledger snapshots omitted reservation identities; the ledger had no restore contract. The loop always created fresh admission history and accumulated events until return. See [ledger](../../src/v2/budget.ts) and [loop](../../src/v2/loop.ts). | Open before stable resumable execution: provide durable checkpoints that preserve budget and admission state across interruption. |
