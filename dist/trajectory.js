 ...
 */
export function normalizeMetrics(metrics, sloc) {
 ...
 */
export function computeQualityScore(normalized, weights = {}) {
 ...
 */
export function buildTrajectory(cache) {
    // Extract entries and sort by timestamp
    const entries = Object.entries(cache.entries).sort((a, b) => a[1].timestamp - b[1].timestamp);
    if (entries.length === 0) {
        return {
            points: [],
            totalDescent: 0,
    ...
    const averageStepSize = points.length > 1 ? totalAbsChange / (points.length - 1) : 0;
    const convergenceState = analyzeConvergence(points, monotonicSteps, regressionSteps, averageStepSize);
    return {
        points,
        totalDescent,
        averageStepSize,
 ...
 */
function analyzeConvergence(points, monotonicSteps, regressionSteps, averageStepSize) {
    if (points.length < 2) {
        return 'stagnating';
    }
    ...
    if (lastPoint.passed && lastPoint.qualityScore > 70) {
        ...
        const recentVariance = recentSteps.length > 1
 ...
 */
export function trajectorySparkline(trajectory) {
 ...
 */
export function formatTrajectorySummary(trajectory) {
    const lines = [];
