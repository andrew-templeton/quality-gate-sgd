 ...
 */
export function getSeverityWeight(metricPath, customWeights) {
 ...
 */
export function sumSeverityWeights(failedRules, customWeights) {
    let total = 0;
 ...
 */
export function normalizeSeverityScore(severityScore, maxExpectedScore = 500) {
 ...
 */
export function computeGradientDirection(failedRules, customWeights) {
    return failedRules
        .map((f) => ({
        rule: f.rule.includes(':') ? f.rule.split(':')[1] : f.rule,
        weight: getSeverityWeight(f.rule.includes(':') ? f.rule.split(':')[1] : f.rule, customWeights),
    }))
