 ...
 */
export function formatSymbolIssues(entry, rank) {
    const lines = [];
    const { symbol, coverage, issues, issueDensity, coverageGap, totalDeltaQ, dependentCount, weightedDeltaQ, fixabilityScore, adjustedDeltaQ, callersCount, calleesCount, } = entry;
    // Header with rank
    const header = rank !== undefined
    ...
    if (issues.sonarqube.length > 0) {
        ...
        const parts = [];
        if (bugs.length > 0)
            parts.push(`${bugs.length} bug${bugs.length !== 1 ? 's' : ''}`);
        if (vulns.length > 0)
            parts.push(`${vulns.length} vulnerability${vulns.length !== 1 ? 'ies' : ''}`);
        if (smells.length > 0)
            parts.push(`${smells.length} smell${smells.length !== 1 ? 's' : ''}`);
        lines.push(`    - SonarQube: ${parts.join(', ')}`);
    }
 ...
 */
export function formatSymbolIssuesList(entries, options = {}) {
 ...
 */
export function formatSymbolIssuesForJson(entries) {
