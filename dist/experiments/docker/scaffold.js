...
import { generateDockerCompose, generateGateConfig, generateExperimentDefinition, generateExperimentReadme, } from './templates.js';
 ...
 */
export function getExperimentDirs(experimentId, baseDir = DEFAULT_EXPERIMENTS_DIR) {
 ...
 */
export function getRunDirs(experimentDirs, runId) {
 ...
 */
export function createExperimentScaffold(options) {
    const { baseDir = DEFAULT_EXPERIMENTS_DIR, name, description, design, agentType, customImage, taskSource, includeExample = true, } = options;
...
export function initializeRun(options) {
    const { experimentId, conditionName, taskId, runId = generateId('run'), cleanupOnComplete = true, timeout, baseDir, } = options;
    // Find experiment
 ...
 */
export function cleanWorkspace(runDirs) {
 ...
 */
export async function cloneToWorkspace(runDirs, repo) {
 ...
 */
export function copyToWorkspace(runDirs, files) {
 ...
 */
export function listExperiments(baseDir = DEFAULT_EXPERIMENTS_DIR) {
 ...
 */
export function listRuns(experimentId, baseDir = DEFAULT_EXPERIMENTS_DIR) {
    const experimentDirs = getExperimentDirs(experimentId, baseDir);
 ...
 * Load a specific run.
 */
export function loadRun(experimentId, runId, baseDir = DEFAULT_EXPERIMENTS_DIR) {
    const experimentDirs = getExperimentDirs(experimentId, baseDir);
    const runDirs = getRunDirs(experimentDirs, runId);
    const runStatePath = path.join(runDirs.root, 'run-state.json');
    if (!fs.existsSync(runStatePath)) {
 ...
 * Update run state.
 */
export function updateRunState(experimentId, runId, updates, baseDir = DEFAULT_EXPERIMENTS_DIR) {
