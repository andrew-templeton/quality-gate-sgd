 ...
 */
function resample(values, newLength) {
    const result = [];
 ...
 */
export function visualizeRun(run) {
    const lines = [];
 ...
 */
export function compareRuns(baseline, treatment) {
    const lines = [];
    lines.push('┌─────────────────────────────────────────────────────┐');
 ...
 */
export function visualizeBatch(batch) {
    const lines = [];
    lines.push('╔════════════════════════════════════════════════════════╗');
 ...
 */
export function visualizeResults(results) {
    const lines = [];
    lines.push('╔══════════════════════════════════════════════════════════════╗');
 ...
 */
export function boxPlot(values, options = {}) {
    ...
    const range = max - min || 1;
    // Map to positions
    const mapPos = (v) => Math.round(((v - min) / range) * (width - 1));
 ...
 */
export function comparativeBoxPlots(data) {
    const lines = [];
    // Find global min/max
...
export function iterationTimeline(iterations) {
    ...
    for (const iter of iterations.slice(0, 20)) {
        ...
        lines.push(`${iter.iteration.toString().padStart(3)}. ${passIcon} Q=${iter.qualityScore.toFixed(2).padStart(6)} ${deltaBar}${deltaStr.padStart(6)}${targetInfo}`);
    }
