...
async function fetchPackageDocs(packageName) {
    try {
        const result = execSync(`npm view ${packageName} readme 2>/dev/null | head -200`, { encoding: 'utf-8', timeout: 10000 });
...
function callClaude(prompt) {
    try {
        ...
        const result = execSync(`echo ${JSON.stringify(prompt)} | claude --print 2>/dev/null`, {
            encoding: 'utf-8',
...
function parseAndValidateResponse(response) {
    ...
    try {
        ...
        const config = {
            ...
            continuity: ['smooth', 'discrete', 'binary'].includes(parsed.continuity)
                ? parsed.continuity
                : 'discrete',
            ...
            extractor: {
                ...
                parseOutput: ['number', 'json', 'regex'].includes(extractor.parseOutput)
                    ? extractor.parseOutput
                    : 'number',
 ...
 */
export async function buildDimension(options) {
 ...
 */
export function appendToConfigFile(config, configPath) {
...
`;
    ...
    if (match) {
        ...
        const newContent = content.replace(match[0], `export const customDimensions: CustomDimensionConfig[] = [${arrayContent ? '\n  ' + arrayContent + ',' : ''}
  ${JSON.stringify(config, null, 2).split('\n').map((line, i) => i === 0 ? line : '  ' + line).join('\n')},
];`);
