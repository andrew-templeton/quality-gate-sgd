 ...
 */
function discoverFiles(rootDir, include, exclude) {
    const files = [];
    function walk(dir) {
        if (!existsSync(dir))
            return;
 ...
 */
function createSymbolId(file, qualifiedName) {
    return `${file}::${qualifiedName}`;
}
/**
 * Extract symbols from a single source file.
 */
function extractSymbolsFromFile(sourceFile, options) {
    const symbols = [];
...
export function extractSymbols(options = {}) {
    const { rootDir = process.cwd(), include = ['**/*.ts', '**/*.tsx'], exclude = ['**/node_modules/**', '**/*.d.ts', '**/dist/**'], } = options;
    // Discover files
 ...
 */
export function extractSymbolsFromSingleFile(filePath) {
