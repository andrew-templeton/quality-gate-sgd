...
import { bindInput, defineAssertion, defineOutput, parseSchema, schema } from './contracts.js';
 ...
 */
export function surfaceWindowStarts(state) {
    ...
    const add = (value) => { boundaries.add(clamp(Math.floor(value))); boundaries.add(clamp(Math.ceil(value))); };
    for (const anchor of state.anchors)
        add(anchor.top - state.chromeHeight);
    ...
    for (let index = 1; index < ordered.length; index++)
        result.add(Math.floor((ordered[index - 1] + ordered[index]) / 2));
...
export function surfaceVisible(state, scrollTop) {
...
export function surfaceRenderDigest(evidence) {
    const body = { ...evidence };
    delete body.renderDigest;
    return digest(body);
}
function statePolicy(policy, id) {
    const state = policy.states.find(value => value.id === id);
    requireThat(state, `Unknown surface state: ${id}`);
    return state;
}
function elementPolicy(policy, id) {
    const element = policy.elements.find(value => value.id === id);
    requireThat(element, `Unknown semantic element: ${id}`);
    return element;
}
export function validateSurfacePolicy(supplied) {
    digest(supplied);
    const policy = freezeJson(structuredClone(supplied));
    const fields = (value, allowed, label) => requireThat(Object.keys(value).length === allowed.length && Object.keys(value).every(key => allowed.includes(key)), `${label} has missing or unsupported fields`);
    ...
    text(policy.version, 'Surface policy version');
    sha(policy.collectorDigest, 'Collector digest');
    unique(policy.knownConcepts, 'Known concepts');
    ...
    unique(policy.states.map(state => state.id), 'Required surface states');
    unique(policy.elements.map(element => element.id), 'Semantic inventory IDs');
    unique(policy.elements.map(element => element.address), 'Semantic addresses');
    for (const element of policy.elements) {
        fields(element, ['id', 'address', 'text', 'concepts', 'defines'], 'Element policy');
        text(element.text, 'Expected semantic text');
        unique(element.concepts, 'Element concepts');
        unique(element.defines, 'Element definitions');
    ...
    for (const state of policy.states) {
        ...
        text(state.route, 'Entry route');
        text(state.interaction, 'Interaction state');
        requireThat(state.input === 'keyboard' || state.input === 'touch', 'Supported input method required');
        for (const size of [state.viewport.width, state.viewport.height])
            requireThat(Number.isSafeInteger(size) && size > 0, 'Positive integral viewport required');
        for (const cap of [state.maxTotal, state.maxNovel])
            requireThat(Number.isSafeInteger(cap) && cap >= 0, 'Semantic caps must be nonnegative safe integers');
        for (const minimum of [state.minFontPx, state.minTargetPx, state.maxBlankGapPx, state.maxScrollScreens])
            requireThat(Number.isFinite(minimum) && minimum > 0, 'Positive geometry limits required');
        for (const ids of [state.requiredElements, state.entryElements, state.actions]) {
            unique(ids, 'State inventory');
            requireThat(ids.every(id => policy.elements.some(element => element.id === id)), 'Unknown state element');
        }
...
export function validateSurfaceEvidence(supplied, policy, identity) {
    const report = parseSchema(surfaceEvidenceSchema, supplied);
    for (const id of [report.sourceDigest, report.dataDigest, report.collectorDigest, report.policyDigest, report.renderDigest, report.artifactDigest, report.environmentDigest])
        sha(id, 'Surface identity');
    ...
    for (const state of report.states) {
        ...
        unique(state.elements.map(element => element.id), 'Measured element IDs');
        unique(state.windows.map(window => window.id), 'Measured window IDs');
        unique(state.anchors.map(anchor => anchor.id), 'Natural anchor IDs');
        for (const element of state.elements)
            requireThat(policy.elements.some(value => value.id === element.id && value.address === element.address), 'Measured element lacks a trusted semantic address');
        ...
        for (const capture of state.captures)
            sha(capture.digest, 'Screenshot digest');
...
export function surfaceWindows(state, policy) {
...
function result(failures, report) {
...
export function surfaceEvidenceModule(options) {
    ...
    const make = (id, claim, inspect) => {
        const card = { id, version: '1', title: id, path: ['communication', id === 'surface.geometry' ? 'geometry' : 'legibility', id.slice(id.indexOf('.') + 1)], claim, evidenceKind: 'deterministic',
    ...
    const geometry = make('surface.geometry', 'Required supplied renders satisfy the configured geometry and interaction predicates.', (report, trusted) => report.states.flatMap(state => {
        ...
        for (const element of state.elements.filter(value => value.rects.length))
            if (element.fontPx < required.minFontPx)
                failures.push({ address: element.address, message: `Text ${element.fontPx}px below ${required.minFontPx}px` });
        for (const action of state.interactions)
            if (!action.reachable || action.method === 'keyboard' && !action.focusVisible || !action.activated || !action.targetVisible || action.targetWidth < required.minTargetPx || action.targetHeight < required.minTargetPx)
                failures.push({ address: action.id, message: `Action fails ${action.method} access, focus, effect, visibility or ${required.minTargetPx}px target minimum` });
        if (state.documentHeight / state.viewport.height > required.maxScrollScreens)
            failures.push({ address: state.id, message: `Scroll burden ${state.documentHeight / state.viewport.height} screens exceeds ${required.maxScrollScreens}` });
        ...
        for (const [top, bottom] of intervals) {
            if (top - end > required.maxBlankGapPx)
                failures.push({ address: state.id, message: `Unaccounted blank gap ${top - end}px exceeds ${required.maxBlankGapPx}px` });
    ...
    const inventory = make('surface.inventory', 'Required semantic elements and entry commitments remain present with their reviewed text and addresses.', (report, trusted) => report.states.flatMap(state => {
        const required = statePolicy(trusted, state.id);
        const failures = [];
        for (const id of required.requiredElements)
            if (!state.elements.some(value => value.id === id && value.rects.length))
                failures.push({ address: id, message: `Required element absent in ${state.id}` });
        for (const id of required.entryElements)
            if (!state.windows[0].visible.includes(id))
                failures.push({ address: id, message: `Material entry element is outside the entry viewport in ${state.id}` });
        for (const element of state.elements)
            if (element.text !== elementPolicy(trusted, element.id).text)
                failures.push({ address: element.address, message: 'Rendered text differs from the independently reviewed inventory' });
        return failures;
    }));
