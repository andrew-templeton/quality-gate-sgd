...
export function traceNudgePlan(nudges, protectedAssertions, context) {
    digest(nudges);
    digest(context);
    nudges = structuredClone(nudges);
    text(context.artifactDigest, 'Baseline artifact digest');
    text(context.scope, 'Resolution scope');
    ...
    const conflicts = plan.conflicts.map(conflict => {
        const left = byId.get(conflict.left);
        const right = byId.get(conflict.right);
        ...
        return { ...conflict, resolution: conflict.preferred ? 'conditional-preference' : 'unresolved', requiredFollowUp: followUp };
    });
    const entries = nudges.map(nudge => {
        ...
        return { id: nudge.id, status: deferred ? 'deferred' : 'selected-for-consideration',
            ...
            claimedEffects: nudge.effects.map(effect => ({ ...effect, verifiedByPlanner: false })), requiredFollowUp,
            remedy: nudge.remediation ? { ...nudge.remediation, executionAuthorized: false } : null };
    });
    const body = { kind: 'partial-nudge-resolution', context: { ...context }, protectedAssertions: [...protectedAssertions],
        plan, entries, conflicts, unresolved: conflicts.filter(item => item.resolution === 'unresolved'), executionAuthorized: false,
...
//# sourceMappingURL=nudge-trace.js.map