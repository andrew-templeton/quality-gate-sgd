...
function probability(value) { requireThat(Number.isFinite(value) && value >= 0 && value <= 1, 'Probability must lie in [0,1]'); }
function distribution(values) { values.forEach(probability); requireThat(Math.abs(values.reduce((a, b) => a + b, 0) - 1) <= tolerance, 'Probability mass must sum to one'); }
function exactKeys(value, ids) { requireThat(Object.keys(value).length === ids.length && ids.every(id => Object.hasOwn(value, id)), 'Provide exactly one value per state'); }
function entropy(values) { return -values.reduce((sum, p) => sum + (p > 0 ? p * Math.log2(p) : 0), 0); }
/** Finite one-assessment EVSI under supplied priors/likelihoods. No inferred causal effect or global policy optimality. */
export function assessExperiment(model, experiment) {
    for (const value of [model.unit, model.perspective, model.horizon, model.basis, experiment.id, experiment.basis])
        text(value, 'Decision context');
    ...
    unique(model.states.map(state => state.id), 'State IDs');
    unique(model.actions.map(action => action.id), 'Action IDs');
    unique(experiment.outcomes.map(outcome => outcome.id), 'Outcome IDs');
    distribution(model.states.map(state => state.prior));
    finite(experiment.cost, 'Experiment cost');
    requireThat(experiment.cost >= 0, 'Cost must be nonnegative');
    const ids = model.states.map(state => state.id);
    for (const action of model.actions) {
        exactKeys(action.utility, ids);
        Object.values(action.utility).forEach(value => finite(value, 'Utility'));
    }
    for (const outcome of experiment.outcomes) {
        exactKeys(outcome.likelihood, ids);
        Object.values(outcome.likelihood).forEach(probability);
    }
    ...
    const outcomes = experiment.outcomes.map(outcome => {
        ...
        if (p === 0)
            return { id: outcome.id, probability: 0, posterior: null, utility: null, actions: [], entropy: null };
        const belief = joints.map(joint => joint / p);
        const choice = best(belief);
    ...
    return {
        kind: 'modeled-one-step', experimentId: experiment.id, unit: model.unit,
...
export function chooseAssessment(model, experiments) {
    if (!model)
        return { kind: 'insufficient-model', selected: [], reason: 'Supply utility context, priors, feasible actions, payoffs and likelihoods before claiming optimal assessment value.' };
    ...
    return { kind: maximum > tolerance ? 'assess' : 'stop',
