...
function observed(failures, evidence, cost) {
...
export function renderedLegibilityModule(options) {
    digest(options);
    ...
    const evidence = (report, artifact, environment, required) => {
        ...
        async evaluate(context, report, config) {
            ...
            for (const view of report.views) {
                ...
                for (const fold of view.folds) {
                    unique(fold.quanta, 'Fold inventory');
                    unique(fold.novel, 'Novel inventory');
                    unique(fold.unexplained, 'Unexplained inventory');
