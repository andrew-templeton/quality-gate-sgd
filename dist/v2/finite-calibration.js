import { validateAssertionContract } from './contracts.js';
import { digest, freezeJson, requireThat, text, unique, validateCost } from './validation.js';
const hash = (value) => requireThat(/^[a-f0-9]{64}$/.test(value), 'Expected SHA-256 identity');
const nonemptyEvidence = (values) => {
 ...
 */
export function assessFiniteVerifier(protocol, observations, evaluator) {
    digest(protocol);
    digest(observations);
    validateAssertionContract(evaluator);
    ...
    hash(protocol.evaluatorDigest);
    hash(protocol.applicabilityDigest);
    ...
    for (const item of protocol.population) {
        ...
        nonemptyEvidence(item.labelEvidence);
        requireThat(['acceptable', 'unacceptable'].includes(item.label), 'Unknown reference label');
    }
    for (const threshold of [protocol.thresholds.maximumFalseAcceptRate, protocol.thresholds.maximumFalseRejectRate])
        requireThat(Number.isFinite(threshold) && threshold >= 0 && threshold <= 1, 'Error-rate thresholds must lie in [0,1]');
    for (const value of [protocol.consequences.falseAccept, protocol.consequences.falseReject])
        requireThat(Number.isFinite(value) && value >= 0, 'Error consequences must be finite and nonnegative');
    ...
    const actualCost = Object.create(null);
    for (const item of observations) {
        ...
        nonemptyEvidence(item.evidence);
        validateCost(item.actualCost);
        if (item.verdict === 'unavailable')
            text(item.reason, 'Unavailable verdict reason');
    ...
    const falseAccept = classResult('unacceptable');
    const falseReject = classResult('acceptable');
    ...
    const body = {
        ...
        status: !complete ? 'incomplete-census' : meets ? 'meets-finite-thresholds' : 'fails-finite-thresholds',
...
//# sourceMappingURL=finite-calibration.js.map