...
import { parseSchema, prepareInput, validateOutputEvidence } from './contracts.js';
    ...
    for (const assertion of gate.assertions) {
        ...
        for (const [name, binding] of Object.entries(assertion.contract?.prerequisites ?? {})) {
            ...
            try {
                ...
                missingOutput = error instanceof Error ? error.message : String(error);
                break;
            }
        }
        if (missingOutput) {
            unavailable(missingOutput);
            results[results.length - 1].reasonCode = 'prerequisite-blocked';
            continue;
        }
        ...
        try {
            ...
            if (assertion.contract?.output && observation.status === 'pass') {
                ...
                requireThat(!Object.hasOwn(observation, 'output'), 'Assertion output requires an explicit output contract');
