...
import { bindInput, defineOutput, defineAssertion, parseSchema, schema } from './contracts.js';
...
export const COMMITMENT_SCHEMA = schema.object({
...
const relationship = schema.union(schema.object({ id: nonempty, kind: schema.literal('sum'), resultId: nonempty,
    terms: schema.array(schema.object({ commitmentId: nonempty, coefficient: schema.number() }), { minItems: 1 }), absoluteTolerance: schema.number({ minimum: 0 }) }), schema.object({ id: nonempty, kind: schema.literal('ratio'), resultId: nonempty, numeratorId: nonempty, denominatorId: nonempty,
    scale: schema.union(schema.literal(1), schema.literal(100)), absoluteTolerance: schema.number({ minimum: 0 }) }), schema.object({ id: nonempty, kind: schema.literal('all-in-cost'), resultId: nonempty, absoluteTolerance: schema.number({ minimum: 0 }) }));
...
export const CANDIDATE_COMMITMENT_SCHEMA = schema.object({
...
const implementation = { id: 'quality-sgd/source-commitments', version: '1', digest: createHash('sha256').update(['commitments', 'contracts', 'validation'].map(name => readFileSync(new URL(`./${name}${extension}`, import.meta.url), 'utf8')).join('\n')).digest('hex') };
const address = (source, part) => `quality://claim/${encodeURIComponent(source.id)}/${encodeURIComponent(part)}`;
function sha(value, label) { requireThat(/^[a-f0-9]{64}$/.test(value), `${label} must be SHA-256`); }
function validatePolicy(policy) {
    ...
    for (const qualifier of policy.semanticEvaluators) {
        sha(qualifier.evaluatorDigest, 'Semantic evaluator identity');
        sha(qualifier.applicabilityDigest, 'Semantic applicability identity');
        text(qualifier.scope, 'Semantic qualification scope');
        requireThat(Array.isArray(qualifier.evidence), 'Qualification evidence required');
        qualifier.evidence.forEach(e => text(e, 'Semantic qualification evidence'));
    }
}
function inventory(items) {
    unique(items.map(c => c.id), 'Commitment IDs');
    unique(items.map(c => c.address), 'Commitment addresses');
    return items.map(item => {
        for (const entries of [item.sourceAddresses, item.conditions, item.dependencies, item.adverseScenarios])
            unique(entries, 'Commitment facet identifiers');
        unique(item.materialCosts.map(c => c.id), 'Material cost IDs');
        if (item.action)
            unique(item.action.conditions, 'Action conditions');
...
export function commitmentInventoryDigest(items) { return digest(inventory(items)); }
export function sourceCoverageDigest(source) {
...
export function sourceReferenceDigest(source) {
...
function effectiveReference(source) {
    ...
    for (const correction of source.corrections) {
        ...
        requireThat(previous && correction.sourceRevisionDigest === source.revisionDigest && correction.before.id === correction.commitmentId && correction.after.id === correction.commitmentId, 'Source correction must bind the exact source revision and preserve the commitment ID');
        ...
        text(correction.approvedBy, 'Correction approver');
        text(correction.reason, 'Correction reason');
...
function numericalFailures(items, relationships) {
    const failures = [];
    const get = (id) => {
        const c = items.find(c => c.id === id);
        requireThat(c?.quantity, `Missing quantity for ${id}`);
        return { ...c, quantity: c.quantity };
    };
    for (const relation of relationships) {
        try {
            const result = get(relation.resultId);
            const q = result.quantity;
            let expected;
            if (relation.kind === 'sum')
                expected = relation.terms.reduce((total, term) => {
                    ...
                    requireThat(input.quantity.unit === q.unit && input.period === result.period && input.population === result.population &&
                        digest(input.quantity.denominator) === digest(q.denominator), 'Sum operands must use the same unit, denominator, period and population; conversions must be explicit upstream');
                    return total + term.coefficient * input.quantity.amount;
                }, 0);
            else if (relation.kind === 'ratio') {
                const numerator = get(relation.numeratorId);
                const denominator = get(relation.denominatorId);
                requireThat(numerator.quantity.denominator === null && denominator.quantity.denominator === null, 'Ratio operands must be unnormalized quantities; nested denominators require explicit upstream conversion');
                requireThat(numerator.quantity.unit === denominator.quantity.unit && numerator.period === denominator.period &&
                    numerator.population === denominator.population && numerator.period === result.period && denominator.population === result.population, 'Ratio operands require a shared counted unit, period and population');
            ...
            if (Math.abs(q.amount - expected) > relation.absoluteTolerance)
                failures.push({ id: relation.id,
                    message: `Declared ${q.amount} ${q.unit} differs from calculated ${expected} by more than ${relation.absoluteTolerance}.` });
        }
        catch (error) {
            failures.push({ id: relation.id, message: error instanceof Error ? error.message : String(error) });
        }
    }
    return failures;
}
/** Compare validated structured annotations. Prose fidelity additionally requires an accepted scoped semantic receipt. */
export function assessCommitments(sourceInput, candidateInput, policy, candidateText) {
    validatePolicy(policy);
    text(candidateText, 'Current candidate text');
    const source = parseSchema(sourceSchema, sourceInput);
    const candidate = parseSchema(CANDIDATE_COMMITMENT_SCHEMA, candidateInput);
    const reference = effectiveReference(source);
    const actual = inventory(candidate.commitments);
    const sourceDigest = sourceCoverageDigest(source);
    const referenceDigest = sourceReferenceDigest(source);
    const evidence = [`commitments:${digest({ source: referenceDigest, candidate, candidateText, policy })}`];
    const summary = { sourceRevisionDigest: source.revisionDigest, sourceContentDigest: source.contentDigest,
    ...
    const finding = (at, message) => ({ address: at, message, evidence });
    const unavailable = (message) => ({ status: 'unavailable', findings: [finding(address(source, 'evidence'), message)], evidence, summary });
    ...
    if (source.contentDigest !== digest(source.content))
        return unavailable('Full source content does not match its recorded digest.');
    if (source.revisionDigest !== candidate.sourceRevisionDigest || sourceDigest !== candidate.sourceCoverageDigest)
        return unavailable('Source revision or coverage binding is stale.');
    if (candidate.candidateContentDigest !== digest(candidateText))
        return unavailable('Candidate annotations do not bind the current text content.');
    const sectionIds = source.sections.map(s => s.address);
    unique(sectionIds, 'Source section addresses');
    unique(source.coverage.coveredAddresses, 'Source coverage addresses');
    ...
    if (!contiguous || source.coverage.status !== 'complete' || !source.coverage.evidence.length ||
        ...
        [...source.commitments, ...reference].some(c => c.sourceAddresses.some(a => !sectionIds.includes(a))))
        return unavailable('Complete coverage of the supplied full source is required.');
    const sourceErrors = numericalFailures(reference, source.relationships);
    if (sourceErrors.length)
        return { status: 'unavailable', findings: sourceErrors.map(e => finding(address(source, `source-arithmetic/${e.id}`), `Source arithmetic is unresolved: ${e.message} Record an explicit source correction before assessing preservation.`)), evidence, summary };
    if (policy.mode === 'qualified-prose') {
        ...
        if (!receipt || !qualification || receipt.status === 'unavailable' || !receipt.evidence.length)
            return unavailable('Required scoped semantic judgment evidence is unavailable or unqualified.');
        ...
        if (Object.entries(bindings).some(([field, expected]) => receipt[field] !== expected))
        ...
        if (receipt.status === 'fail')
            return { status: 'fail', findings: [finding(address(source, 'semantic-judgment'), 'The accepted scoped semantic evaluator rejected preservation of the corrected source commitments.')], evidence, summary };
    }
    const findings = [];
    const compared = ['address', 'sourceAddresses', 'proposition', 'quantity', 'actor', 'population', 'period', 'scope', 'conditions', 'dependencies',
        'adverseScenarios', 'uncertainty', 'claimStrength', 'materialCosts', 'action'];
    for (const expected of reference) {
        const item = actual.find(c => c.id === expected.id);
        if (!item) {
            findings.push(finding(expected.address, 'A required source commitment is missing.'));
            continue;
        }
        const changes = compared.filter(field => digest(expected[field]) !== digest(item[field]));
        if (!changes.length)
            summary.preservedCommitmentIds.push(expected.id);
        for (const field of changes)
            findings.push(finding(`${expected.address}/${field}`, `Source commitment ${field} changed: expected ${JSON.stringify(expected[field])}; observed ${JSON.stringify(item[field])}. Preserve the registered meaning or record an explicit source correction.`));
    }
    for (const extra of actual.filter(c => !reference.some(s => s.id === c.id)))
        findings.push(finding(extra.address, 'Candidate introduces an unregistered commitment; it requires an explicit reference update.'));
    for (const error of numericalFailures(actual, source.relationships))
        findings.push(finding(address(source, `candidate-arithmetic/${error.id}`), error.message));
...
export function sourceCommitmentsModule(options) {
    ...
    validatePolicy(policy);
    sha(source.revisionDigest, 'Source revision');
    sha(source.contentDigest, 'Source content');
    ...
    validateCost(cost);
    const candidateTextPath = options.candidateTextPath ?? ['text'];
    candidateTextPath.forEach(p => text(p, 'Candidate text path'));
    ...
    const assertion = defineAssertion({
        ...
        async evaluate(context, report, config) {
            let content = context.artifact.data;
            for (const part of config.candidateTextPath) {
                if (content === null || typeof content !== 'object' || !Object.hasOwn(content, part))
                    return {
            ...
            if (typeof content !== 'string' || content.trim().length === 0 || report.candidateRevisionDigest !== context.artifact.digest)
                return {
            ...
            let result;
            try {
                result = assessCommitments(config.source, report, config.policy, content);
            }
            catch (error) {
                return { status: 'unavailable', findings: [{ address: 'quality://claim/candidate/commitments',
