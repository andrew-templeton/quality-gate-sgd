...
function parseArgs(args: string[]): ParsedArgs {
  const result: ParsedArgs = { limit: 100, summary: false };
...
async function fetchIssues(options: ParsedArgs): Promise<SonarResponse> {
...
function groupByFile(
  issues: SonarIssue[],
  fileFilter?: string
): Map<string, Array<{ line?: number; message: string; rule: string }>> {
  const grouped = new Map<
    string,
    Array<{ line?: number; message: string; rule: string }>
  >();
...
`);
  ...
  console.log(
    `Fetching issues from SonarQube${filters ? ` (${filters})` : ''}...`
  );
  console.log();
  ...
  try {
    ...
    const displayedCount = [...grouped.values()].reduce(
    ...
    for (const issue of response.issues) {
      const file = formatComponent(issue.component);
      if (options.file && !file.includes(options.file)) continue;
