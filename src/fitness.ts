...
import { getAllDimensions, getDimension, type DimensionDef } from './dimensions/index.js';
 ...
 */
export function getMetricValue(metrics: Metrics, path: string): number | undefined {
 ...
 */
function normalizeValue(value: number, dim: DimensionDef): number {
 ...
 */
export function computeFitness(metrics: Metrics, config?: FitnessConfig): number {
...
function computeGeometricMeanFitness(
  metrics: Metrics,
...
export function computeGradient(
  ...
  const effectiveConfig = config ?? getDefaultFitnessConfig();
  const dimensions = getAllDimensions();
  const components: GradientComponent[] = [];
 ...
 */
export function suggestNextFixes(
  metrics: Metrics,
  limit = 5,
  config?: FitnessConfig
): FitnessSuggestion[] {
  const gradient = computeGradient(metrics, config);
  const suggestions: FitnessSuggestion[] = [];
  ...
  for (const comp of gradient.slice(0, limit)) {
    ...
    if (dim.direction === 'higher-better' && comp.currentValue >= 99) continue;
    if (dim.direction === 'lower-better' && comp.currentValue <= 0) continue;
 ...
 */
export function formatFitnessScore(score: number): string {
 ...
 */
export function formatGradientTable(gradient: GradientComponent[]): string {
  if (gradient.length === 0) {
  ...
  for (const comp of gradient) {
    ...
    const value = comp.direction === 'higher-better'
      ...
      : `${comp.currentValue}`;
    lines.push(
      `| ${comp.displayName} | ${value} | ${dir} | ${comp.estimatedImprovement.toFixed(4)} | ${comp.priority.toFixed(4)} |`
    );
  }
 ...
 */
export function formatSuggestion(suggestion: FitnessSuggestion): string {
  const lines: string[] = [
