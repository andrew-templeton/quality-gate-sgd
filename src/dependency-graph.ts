 ...
 */
export function getAllTypeScriptFiles(dir: string, files: string[] = []): string[] {
 ...
 */
export function calculateDegrees(files: string[], srcDir: string): Map<string, FileInfo> {
  ...
  while (changed && iteration < maxIterations) {
    ...
    for (const [, info] of fileInfoMap) {
      if (info.degree !== -1) continue; // Already calculated
      ...
      if (info.localDependencies.length === 0) {
        ...
        const depDegrees = info.localDependencies.map(
          (dep) => fileInfoMap.get(dep)?.degree ?? -1
        );
  ...
  const maxDegree = Math.max(
    ...[...fileInfoMap.values()].map((f) => f.degree).filter((d) => d !== -1),
    0
  );
 ...
 */
export function buildDependentCounts(files: Map<string, FileInfo>): void {
  ...
  const maxIndirect = Math.max(
    ...[...files.values()].map((f) => f.indirectDependents),
 ...
 */
export function attachCoverageData(
  ...
  try {
    coverage = JSON.parse(fs.readFileSync(coveragePath, 'utf-8')) as CoverageSummary;
  } catch {
    return;
  }
