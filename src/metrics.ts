...
function completedMetricProcess(collector: string, result: SpawnSyncReturns<string>): number {
  if (result.error) {
...
function isEslintFileResult(value: unknown): value is EslintFileResult {
  ...
  const file = value as Partial<EslintFileResult>;
  if (typeof file.filePath !== 'string' || !file.filePath.trim()
    || !Number.isSafeInteger(file.errorCount) || (file.errorCount as number) < 0
    || !Number.isSafeInteger(file.warningCount) || (file.warningCount as number) < 0
    || !Array.isArray(file.messages)) return false;
  for (const message of file.messages) {
    if (!message || typeof message !== 'object'
      || !(message.ruleId === null || typeof message.ruleId === 'string')
      || ![1, 2].includes(message.severity)
      || typeof message.message !== 'string' || !message.message.trim()) return false;
  }
...
export function extractEslintMetrics(): EslintMetrics {
  ...
  catch (error) { throw new Error('ESLint metric collection failed: malformed JSON report', { cause: error }); }
