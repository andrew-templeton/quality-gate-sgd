 ...
 */
function computeCoverageGap(coverage?: CoverageMetrics): number {
  if (!coverage) {
 ...
 */
function computeEaseOfTesting(degree: number): number {
  return 1 / (1 + degree);
}
...
export function computePriority(
  ...
  customSeverityWeights?: Record<string, number>
): PrioritizedFile {
 ...
 */
export function formatPrioritizedFiles(
  prioritizedFiles: PrioritizedFile[],
  srcDir: string,
  limit: number = 20
): string {
  ...
  for (const pf of top) {
    ...
    lines.push(
      `  Components: cov=${pf.components.coverageGap.toFixed(2)} ease=${pf.components.easeOfTesting.toFixed(2)} ` +
