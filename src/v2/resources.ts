...
export function readResource(uri: string, supplied: { modules?: AssertionModule[]; gate?: CompiledGate } = {}): { contents: Array<{ uri: string; mimeType: string; text: string }> } | undefined {
  const response = (value: unknown) => ({ contents: [{ uri, mimeType: 'application/json', text: JSON.stringify(value, null, 2) }] });
  if (uri === RESOURCES[1].uri) return response(catalogContext(supplied.modules ?? []));
  if (uri === RESOURCES[2].uri) return supplied.gate ? response(compositionContext(supplied.gate)) : response({ status: 'unavailable', reason: 'Supply a compiled gate to describe its selected subset.' });
