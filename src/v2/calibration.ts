...
export interface PreferenceCase { id: string; prompt: string; baseline: string; candidate: string }
export interface PreferencePacket {
  schemaVersion: '1';
  id: string;
  question: string;
  pairs: { id: string; prompt: string; A: string; B: string }[];
  digest: string;
}
...
export interface PreferenceAnswerKey {
  ...
  assignments: { pairId: string; caseId: string; candidateSide: Side }[];
  digest: string;
}
...
function probability(value: number, label: string, open = false): void {
  requireThat(Number.isFinite(value) && (open ? value > 0 && value < 1 : value >= 0 && value <= 1), `${label} must lie in ${open ? '(0,1)' : '[0,1]'}`);
}
function validateUse(use: EvidenceUse): boolean {
...
function validateProtocol(protocol: PreferenceProtocol): void {
...
function packetDigest(packet: Omit<PreferencePacket, 'digest'>): string {
  return digest({ schemaVersion: packet.schemaVersion, id: packet.id, question: packet.question, pairs: packet.pairs });
}
function keyDigest(key: Omit<PreferenceAnswerKey, 'digest'>): string {
 ...
 */
export function preparePreferenceTrial(protocol: PreferenceProtocol, cases: PreferenceCase[]): { packet: PreferencePacket; answerKey: PreferenceAnswerKey } {
  ...
  const entries = cases.map(item => {
    ...
    const id = randomUUID();
    const candidateSide: Side = randomInt(2) === 0 ? 'A' : 'B';
    return {
  ...
  const packetBody = { schemaVersion: '1' as const, id: randomUUID(), question: protocol.question, pairs: entries.map(entry => entry.pair) };
  const packet = { ...packetBody, digest: packetDigest(packetBody) };
  const frozenProtocol = structuredClone(protocol);
  const keyBody = { schemaVersion: '1' as const, packetId: packet.id, packetDigest: packet.digest,
...
function validateTrial(packet: PreferencePacket, key: PreferenceAnswerKey): void {
...
function upperErrorBound(errors: number, n: number, tailAlpha: number): number | null {
  if (n === 0) return null;
  if (errors === n) return 1;
  if (errors === 0) return -Math.expm1(Math.log(tailAlpha) / n);
  let lower = 0;
  let upper = 1;
  for (let iteration = 0; iteration < 60; iteration++) {
    const middle = (lower + upper) / 2;
    if (binomialCdf(errors, n, middle) > tailAlpha) lower = middle;
    else upper = middle;
  }
  return upper;
}
 ...
 */
export function assessPreferenceTrial(packet: PreferencePacket, answerKey: PreferenceAnswerKey, responses: PreferenceResponse[], use: EvidenceUse) {
  ...
  for (const response of responses) {
    ...
    if (response.choice === 'tie') ties++;
    else if (response.choice === 'cannot-judge') cannotJudge++;
    else if (response.choice === assignment.candidateSide) wins++;
    else losses++;
  }
  ...
  if (missing > 0) reasons.push(`${missing} responses are missing`);
  if (cannotJudge > 0) reasons.push(`${cannotJudge} pairs could not be judged`);
  if (decisive < protocol.minimumDecisivePairs) reasons.push('Preregistered minimum decisive sample was not reached');
  if (protocol.mode !== 'confirmatory') reasons.push('Protocol was registered for exploration');
  if (!freshIndependentEvidence) reasons.push('Fresh, independent, unadapted first-use evidence was not attested');
  const status = !complete || decisive < protocol.minimumDecisivePairs ? 'insufficient-evidence' as const
    : protocol.mode !== 'confirmatory' || !freshIndependentEvidence ? 'exploratory-only' as const
    : pValue !== null && pValue <= protocol.alpha ? 'supports-preference-improvement' as const
    : 'no-demonstrated-improvement' as const;
  return {
    kind: 'paired-preference' as const, status, packetDigest: packet.digest, protocolDigest: answerKey.protocolDigest,
 ...
 */
export function assessVerifierCalibration(protocol: VerifierCalibrationProtocol, cases: LabeledVerifierCase[], use: EvidenceUse) {
  ...
  for (const item of cases) {
    ...
    if (item.verdict === 'unavailable') { unavailable++; continue; }
    if (item.label === 'acceptable') { acceptable++; if (item.verdict === 'reject') falseRejects++; }
    else { unacceptable++; if (item.verdict === 'accept') falseAccepts++; }
  }
  ...
  const status = !sufficient ? 'insufficient-evidence' as const
    : protocol.mode !== 'confirmatory' || !freshIndependentEvidence ? 'exploratory-only' as const
    : qualifies ? 'meets-error-bounds' as const : 'does-not-meet-error-bounds' as const;
  return {
    kind: 'known-label-verifier-calibration' as const, status, protocolDigest: digest(protocol),
