...
export interface LoopState {
  ...
  lastChanges: (Change & { round: number })[];
  stalledRounds: number;
  lastRound: number;
}
export function initialLoopState(evaluation: Evaluation): LoopState {
...
export function admitCandidate(
  baseline: Evaluation, candidate: Evaluation, nudge: Nudge, round: number,
  policy: AdmissionPolicy, state: LoopState,
): { accepted: boolean; reason: string; state: LoopState } {
  ...
  for (const value of Object.values(policy.objectives)) requireThat(Number.isFinite(value) && value >= 0, 'Improvement margins must be finite and nonnegative');
  const reject = (reason: string) => ({ accepted: false, reason, state: { ...state, lastRound: Math.max(state.lastRound, round), stalledRounds: state.stalledRounds + 1 } });
  try {
    validateNudge(nudge);
  ...
  } catch (error) { return reject(`Invalid evidence: ${error instanceof Error ? error.message : String(error)}`); }
  if (candidate.contractDigest !== baseline.contractDigest || state.contractDigest !== baseline.contractDigest) return reject('Contract changed; rebaseline instead of comparing incomparable runs');
  if (state.acceptedDigests.at(-1) !== baseline.artifactDigest) return reject('Baseline is not the current accepted artifact');
  if (state.acceptedDigests.includes(candidate.artifactDigest)) return reject('Previously accepted artifact: cycle or no-op');
  if (candidate.budget.exceeded) return reject('Cost upper bound exceeded');
  ...
  for (const id of baseline.requiredAssertions) {
    ...
    if (a?.status === 'unavailable' && a.reasonCode === 'prerequisite-blocked') {
      if (b?.status === 'pass') { repaired = true; continue; }
      if (b?.status === 'unavailable' && b.reasonCode === 'prerequisite-blocked') continue;
      return reject(`Previously blocked assertion has no passing comparison: ${id}`);
    }
    if (!a || !b || a.status === 'unavailable' || b.status === 'unavailable') return reject(`Required comparison unavailable: ${id}`);
    if (a.status === 'pass' && b.status !== 'pass') return reject(`Required assertion regressed: ${id}`);
    if (a.status === 'fail' && b.status === 'pass') repaired = true;
  }
  ...
  for (const change of reversals) {
    ...
    if (last && round - last.round <= policy.cooldownRounds) return reject('Opposite nudge is inside the reversal cooldown');
  }
  let improved = false;
  for (const [id, margin] of Object.entries(policy.objectives)) {
    const a = before.get(id); const b = after.get(id);
    if (!a?.loss || !b?.loss || a.status === 'unavailable' || b.status === 'unavailable' || a.loss.unit !== b.loss.unit) return reject(`Objective bounds unavailable or units changed: ${id}`);
    if (b.loss.upper > a.loss.lower) return reject(`No robust nonregression established: ${id}`);
    if (a.loss.lower - b.loss.upper > margin * (reversals.length ? policy.reversalMultiplier : 1)) improved = true;
  }
  if (!repaired && !improved) return reject('No hard repair or robust gain beyond the deadband');
  if (reversals.length && !improved) return reject('A reversal requires measured gain beyond the larger deadband');
