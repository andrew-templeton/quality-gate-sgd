...
export interface Interval { lower: number; upper: number; unit: string }
export interface AssertionCard {
  ...
  path: string[];
  claim: string;
  input: { schemas: string[]; capabilities: string[]; description: string };
...
export interface GatePolicy { required: string[]; advisory: string[] }
...
export interface Evaluation {
  ...
  budget: { limits: Cost; spent: Cost; reserved: Cost; exceeded: boolean };
}
export interface Change { address: string; variable: string; direction: -1 | 1 }
export interface Nudge {
  ...
  reads: string[];
  writes: string[];
  effects: { assertionId: string; direction: 'improves' | 'worsens' | 'unknown'; basis: 'hypothesis' | 'observed' | 'identified'; evidence: string[] }[];
  /** Net benefit AFTER repair cost on an explicitly supplied common utility scale. */
  netBenefit?: Interval;
  costUpperBound: Cost;
  remediation?: { harnessId: string; prompt: string; verification: string[] };
}
