...
import type { AssertionModule, Cost, EvaluationContext, GatePolicy } from './types.js';
...
interface Configuration {
  ...
  selected: string[];
  policy: GatePolicy;
  input: Omit<EvaluationContext, 'signal'>;
  budget: Cost;
...
async function main(): Promise<void> {
  ...
  if (command === 'zoo') { console.log(JSON.stringify(ASSERTION_ZOO, null, 2)); return; }
  requireThat(['discover', 'card', 'run'].includes(command) && configPath, 'Expected zoo, discover, card or run; use --help');
  const config = (await import(pathToFileURL(resolve(configPath)).href)).default as Configuration;
  if (command === 'discover') {
    requireThat(config.available, 'Discovery requires available schema/capability declarations');
    console.log(JSON.stringify(discoverAssertions(config.modules, config.available, rest.join(' ')), null, 2)); return;
  }
  const gate = compileGate(config.modules, config.selected, config.policy);
  if (command === 'card') { console.log(JSON.stringify(modelCard(gate), null, 2)); return; }
