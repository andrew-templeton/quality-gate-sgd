...
export function validateInterval(value: Interval): void {
...
export function validateObservation(value: Observation): void {
  ...
  for (const finding of value.findings) {
    text(finding.address, 'Finding address'); text(finding.message, 'Finding message');
  ...
  if (value.status !== 'unavailable') requireThat(value.evidence.length > 0, 'An evaluated verdict requires evidence');
  if (value.status === 'fail') requireThat(value.findings.length > 0, 'A failed assertion requires an addressed finding');
  if (value.loss) validateInterval(value.loss);
