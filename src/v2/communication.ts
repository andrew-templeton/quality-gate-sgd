...
import { bindInput, defineAssertion, defineOutput, fromAssertion, parseSchema, schema, type SchemaValue } from './contracts.js';
import { COMMITMENT_OUTPUT, sourceReferenceDigest, type SourceCommitmentContract } from './commitments.js';
import { SURFACE_OUTPUT, validateSurfacePolicy, type SurfacePolicy } from './surfaces.js';
...
function validateContract(value: CommunicationContract): CommunicationContract {
  const contract = parseSchema(contractSchema, value);
  for (const key of ['sourceRevisionDigest', 'sourceReferenceDigest', 'surfacePolicyDigest', 'collectorDigest'] as const)
    requireThat(/^[a-f0-9]{64}$/.test(contract[key]), `Communication ${key} must be SHA-256`);
  for (const list of [contract.audience.knownConcepts, contract.audience.unfamiliarConcepts, contract.task.requiredCommitmentIds, contract.task.entryElementIds]) unique(list, 'Communication contract identifiers');
...
export function defineCommunicationContract(options: {
  audience: CommunicationAudience; task: CommunicationTask; source: SourceCommitmentContract;
  fidelityMode: CommunicationContract['fidelityMode']; surfacePolicy: SurfacePolicy;
 ...
 */
export function communicationScopeModule(supplied: CommunicationContract) {
  const contract = validateContract(supplied);
  const contractDigest = digest(contract);
  const extension = import.meta.url.endsWith('.ts') ? '.ts' : '.js';
  const implementation = { id: 'quality-sgd/communication-scope', version: '1', digest: createHash('sha256').update(
    ['communication', 'contracts', 'validation'].map(name => readFileSync(new URL(`./${name}${extension}`, import.meta.url), 'utf8')).join('\n'),
  ).digest('hex') };
  const prerequisites = {
  ...
  const assertion = defineAssertion({
    ...
    async evaluate(context, input, config, upstream) {
      ...
      if (input.contractDigest !== config.contractDigest) failures.push('Communication contract binding changed');
      if (source.sourceRevisionDigest !== config.contract.sourceRevisionDigest || source.referenceDigest !== config.contract.sourceReferenceDigest ||
        source.candidateRevisionDigest !== context.artifact.digest || source.semanticMode !== config.contract.fidelityMode) failures.push('Source evidence does not match the registered source, candidate or fidelity mode');
      if (!config.contract.task.requiredCommitmentIds.every(id => source.preservedCommitmentIds.includes(id))) failures.push('A required task commitment has no passing preservation receipt');
      ...
      if (failures.length) return { status: 'unavailable' as const, findings: failures.map(message => ({ address: `quality://claim/communication/${config.contract.task.id}`, message, evidence })), evidence, actualCost: {} };
      return { status: 'pass' as const, findings: [], evidence, actualCost: {}, output: {
